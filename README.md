...
type CategoryMap = {
		 [Key in  category]: string - позволяет устанавливать категорию в зависимости от класса
};
```
## Описание данных (интерфейсы)

### Интерфейс пользователя IUserInfo

- `payment: 'cash' | 'card'` - тип оплаты
- `address: string` - адрес
- `email: string` - электронный адрес
- `phone: number` - номер телефона

### Интерфейс финальной стоимости ISuccess 

- `total: number` - финальное количество
...
### Интерфейс взаимодействия с карточкой ICardAction 

  - `onClick: (event: MouseEvent) => void` - функция передаваемая по клику
  - `price: number | null` - стоиомсть
  - `title: string` - заголовок
  - `index?: number` - нумерация
